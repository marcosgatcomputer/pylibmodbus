...
ffi.cdef(
    ...
    void modbus_enable_rpi(modbus_t *ctx, int state);
    void modbus_configure_rpi_bcm_pins(modbus_t *ctx, int BCM_PIN_DE, int BCM_PIN_RE);
    void modbus_rpi_pin_export_direction(modbus_t *ctx);
    void modbus_rpi_pin_unexport_direction(modbus_t *ctx);
"""
)
C = ffi.dlopen("modbus")
