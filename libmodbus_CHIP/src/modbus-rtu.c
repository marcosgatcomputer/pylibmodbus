...
static int _set_GPIO_value(int debug,int pin,int value)
{
  ...
  char path[VALUE_MAX];
  int fd;
  snprintf(path, VALUE_MAX+1, "/sys/class/gpio/gpio%d/value",pin);
...
static int _set_GPIO_pin(modbus_t *ctx,int value)
{
  ...
  {
    if(_set_GPIO_value(ctx->debug,ctx->rpi_bcm_pin_de,value) == 0)
    {
     return _set_GPIO_value(ctx->debug,ctx->rpi_bcm_pin_re,value);
     //return 0;
    }
    return(-1);
  }
