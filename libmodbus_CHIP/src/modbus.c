...
int modbus_configure_rpi_bcm_pin(modbus_t *ctx, uint16_t value)
{
  return modbus_configure_rpi_bcm_pins(ctx,value,value);
}

int modbus_configure_rpi_bcm_pins(modbus_t *ctx, uint16_t de, uint16_t re)
{
...
static int _modbus_rpi_pin_export_direction(int debug,uint16_t rpi_bcm_pin)
{
  //export GPIO code
  char buffer[5];
  ...
  bytes_written = snprintf(buffer, 5, "%d",rpi_bcm_pin);
  ...
  snprintf(path, DIRECTION_MAX+1, "/sys/class/gpio/gpio%d/direction", rpi_bcm_pin);
...
static int _modbus_rpi_pin_unexport_direction(int debug,uint16_t rpi_bcm_pin)
{
  //set GPIO direction code
...
int modbus_rpi_pin_export_direction(modbus_t *ctx)
{
  ...
  {
    if(_modbus_rpi_pin_export_direction(ctx->debug,ctx->rpi_bcm_pin_de) == 0)
    {
       //return 0;
       return _modbus_rpi_pin_export_direction(ctx->debug,ctx->rpi_bcm_pin_re);
    }
    else
    {
      return(-1);
...
int modbus_rpi_pin_unexport_direction(modbus_t *ctx)
{
  if(ctx->enable_rpi_rtu == 1)
  {
    if(_modbus_rpi_pin_unexport_direction(ctx->debug,ctx->rpi_bcm_pin_de) == 0)
    {
     return _modbus_rpi_pin_unexport_direction(ctx->debug,ctx->rpi_bcm_pin_re);
     //return 0;
    }
    else
    {
